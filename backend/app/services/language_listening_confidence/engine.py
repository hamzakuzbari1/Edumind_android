"""Confidence-aware listening recommendation orchestration (Phase 3.2)."""
...
def recommend_confidence_aware_listening_plan(
    level: str,
    ...
    *,
    ...
    """Wrap curriculum + goal scoring with confidence-based objective priority (no engine file edits)."""
    ...
    def _build(plan) -> tuple[CurriculumRecommendation, GoalAwareRecommendation, float, float]:
        ...
        blended, conf_w = blend_with_confidence(
            cur_score_total,
            goal_score.total,
            conf_factor,
        ...
        rec = CurriculumRecommendation(
            ...
            selection_reason=(
                f"confidence intent={intent.value} cur={cur_score_total:.2f} "
        ...
        return rec, goal_rec, conf_factor, blended
    ...
    if not candidates:
        ...
        rec, goal_rec, conf_factor, blended = _build(plan)
        return ConfidenceAwareRecommendation(
            recommendation=rec,
            goal_aware=goal_rec,
            confidence_state=confidence_state,
            confidence_boost=conf_factor,
            blended_score=blended,
    ...
    for plan in candidates:
        ...
        if blended > best_blended:
            best_rec, best_goal, best_conf, best_blended = rec, goal_rec, conf_factor, blended
