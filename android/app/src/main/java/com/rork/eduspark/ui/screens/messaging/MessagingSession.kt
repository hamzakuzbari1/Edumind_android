 ...
 * raw session id — see that constant's own doc comment for why. Parent resolves onto the
 * seeded Parent messaging participant only at this boundary; auth/profile/linking ids stay
 * untouched.
 */
fun SessionUser.messagingParticipantIdOrNull(): String? = when (role) {
    UserRole.Teacher -> id
    UserRole.Student -> CURRENT_STUDENT_MESSAGING_ID
    UserRole.Parent -> CURRENT_PARENT_MESSAGING_ID
}
